...
void Asistente::guardarEnArchivo(const string& nombreArchivo) {
    std::ofstream archivo(nombreArchivo, std::ios::app);
    if (archivo.is_open()) {
        archivo << nombre << "," << edad << "," << dinero << "," << universidad << "," << esAlumno << "," << esProfesor << "\n";
        archivo.close();
    } else {
        std::cerr << "Error: No se pudo abrir el archivo para guardar el asistente.\n";
    }
}
